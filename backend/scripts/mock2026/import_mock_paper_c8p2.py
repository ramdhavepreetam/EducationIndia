"""Import the Paper II vendor mock into a fresh scratch exam. Creates new rows on every run."""
import asyncio, json, asyncpg, sys

import os
_HERE=os.path.dirname(os.path.abspath(__file__))
SRC=os.path.join(_HERE,'msce_class8_paper2_mock_2026.json')
...
IMG_PREFIX=f'{BASE_URL}/static/mock2026c8p2/'
...
TOPICS={
 'Marathi':['Vocabulary','Grammar','Reading Comprehension','Poetry','Idioms and Proverbs'],
 'Intelligence Test':['Mirror and Water Images','Analogy','Series Completion','Pattern Recognition',
                      'Direction and Position','Coding and Decoding','Venn Diagrams','Number Puzzles',
                      'Odd One Out','Logic and Reasoning'],
}
SEC={'Marathi':('I',1,25),'Intelligence Test':('II',26,75)}
...
async def main():
    ...
    async with c.transaction():
        ...
        ev=await c.fetchval("""insert into exam_events(board_id,category_id,title_en,title_mr,std_class,year,is_active)
             ...
             board,cat,'SCRATCH — Mock Test 2026 Std 8 Paper II (vendor import)','स्क्रॅच — सराव चाचणी २०२६ इयत्ता ८ पेपर २',
        ...
        exam=await c.fetchval("""insert into exams(event_id,paper_code,set_code,paper_number,title_en,title_mr,medium,
             ...
             values($1,$2,$3,2,$4,$5,'marathi',75,150,2,90,false) returning id""",
             ev,paper['paper_code'],'M8B','Mock Test 2026 — Std 8 Paper II','सराव चाचणी २०२६ — इयत्ता ८ पेपर २')
